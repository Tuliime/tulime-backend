...
func main() {
	...
	userGroup.Get("/online-status", middlewares.Auth, auth.GetUsersOnlineStatus)
	...
	advertGroup.Patch("/:id/price/:advertPriceID", middlewares.Auth, adverts.UpdateAdvertPrice)
	advertGroup.Post("/:id/inventory", middlewares.Auth, adverts.PostAdvertInventory)
	advertGroup.Patch("/:id/inventory/:advertInventoryID", middlewares.Auth, adverts.UpdateAdvertInventory)
